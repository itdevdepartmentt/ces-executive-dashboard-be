...
let QaProductivityService = class QaProductivityService {
    ...
    async getSettings() {
        ...
        settings.forEach(s => {
            if (s.type === 'QC')
                uniqueQcs.add(s.name);
            if (s.type === 'AGENT')
                uniqueAgents.add(s.name);
        });
    ...
    async getDashboardData(month, year, dateStr) {
        const settings = await this.prisma.qaTargetSetting.findMany();
        ...
        const startOfTargetMonth = (0, date_fns_1.startOfMonth)(new Date(year, month - 1));
        const endOfTargetMonth = (0, date_fns_1.endOfMonth)(new Date(year, month - 1));
        ...
        for (const form of monthlyForms) {
            const isEksekutor = (0, date_fns_1.isSameDay)(form.createdAt, form.createdDate);
        ...
        return {
            settings: Array.from(qcSettingsMap.values()).concat(Array.from(agentSettingsMap.values())),
            qcProductivity,
            agentPerformance,
            realtimeOverview: {
                totalAll,
