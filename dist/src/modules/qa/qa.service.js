...
let QaService = class QaService {
    ...
    async createFormTapping(data) {
        ...
        const totalScore = (formTapping.scoreValiditas || 0) +
    ...
    async getPendingTickets(page = 1, limit = 10, search, filters, user, sortBy, sortOrder = 'desc') {
        try {
    ...
    async createExcelBuffer(data, sheetName) {
    ...
    async uploadTickets(file) {
        ...
        if (isXlsx) {
            ...
            worksheet.eachRow((row, rowNumber) => {
                if (rowNumber === 1) {
                    headers = row.values.map(h => h ? String(h).trim() : '');
                }
                else {
                    const rowData = {};
        ...
        try {
            ...
            const finalBatch = uniqueResults.filter(t => !existingPendingIds.has(String(t.idTiket)) && !existingTappedIds.has(String(t.idTiket)));
            ...
            throw new common_1.BadRequestException('Failed to insert tickets into database');
        }
    }
    async getQaScoreDashboard(year, month, agent, peak, user, teamLeader) {
        const whereClause = {};
        if (year) {
    ...
    async getDetailTapping(page = 1, limit = 100, year, month, agent, peak, search, filters, sortBy, sortOrder = 'desc', user, teamLeader) {
        const skip = (page - 1) * limit;
        const whereClause = {};
        const andConditions = [];
        if (year) {
        ...
        const eksekutorPercentage = allForStats.length > 0
            ...
            : 0;
        const censoredData = data.map((row) => {
            const isAgent = user?.name === row.agent;
            const isTl = user?.name === row.teamLeader;
            const isPending = row.komitmenStatus === 'PENDING';
    ...
    async approveKomitmen(id, user) {
        const record = await this.prisma.qaFormTapping.findUnique({ where: { id } });
        if (!record)
            throw new common_1.NotFoundException('Data tidak ditemukan');
        if (user.role === 'TL' && record.teamLeader !== user.name) {
            throw new common_1.UnauthorizedException('Anda tidak berhak melakukan persetujuan untuk agen ini');
        }
        return this.prisma.qaFormTapping.update({
            where: { id },
    ...
    async rejectKomitmen(id, user) {
        const record = await this.prisma.qaFormTapping.findUnique({ where: { id } });
        if (!record)
            throw new common_1.NotFoundException('Data tidak ditemukan');
        if (user.role === 'TL' && record.teamLeader !== user.name) {
            throw new common_1.UnauthorizedException('Anda tidak berhak melakukan penolakan untuk agen ini');
        }
        return this.prisma.qaFormTapping.update({
            where: { id },
